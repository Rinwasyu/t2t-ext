...
let createWorkerFromFunction = function(workerFunc) {
	let functionBody = workerFunc.toString().trim().match(
...
let worker = createWorkerFromFunction(function() {
	onmessage = function(e) {
		if (e.data.md5sum != SparkMD5.hash(e.data.str)) {
			for (let i = 0; i < e.data.regex.length; i++) {
				e.data.str = e.data.str.replace(e.data.regex[i][0], "<span class='kanyou'>" + e.data.regex[i][1]+"</span>");
			}
			e.data.md5sum = SparkMD5.hash(e.data.str);
			postMessage(e.data);
		} else {
			postMessage(false);
		}
...
worker.onmessage = function(e) {
	if (e.data) {
		if (t2t_elements_list[e.data.list][e.data.id]) {
			t2t_elements_list[e.data.list][e.data.id].innerHTML = e.data.str;
			t2t_elements_list[e.data.list][e.data.id].dataset.t2t_md5sum = e.data.md5sum;
...
let get_elements_list = function() {
	if (location.host.match(/^twitter.com$/)) {
		return [document.getElementsByClassName("css-901oao r-hkyrab r-1tl8opc r-16dba41 r-ad9z0x r-bcqeeo r-bnwqim r-qvutc0")];
	} else if (location.host.match(/^.*\.?open2ch.net$/)) {
...
let t2t = function() {
	...
	t2t_elements_list = get_elements_list();
